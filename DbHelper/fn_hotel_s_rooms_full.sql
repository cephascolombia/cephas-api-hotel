CREATE OR REPLACE FUNCTION public.fn_hotel_s_rooms_full(p_page integer, p_page_size integer, p_name character varying, p_capacity integer, p_min_price numeric, p_max_price numeric, p_sort character varying)
 RETURNS TABLE(room_id integer, name character varying, price_per_night numeric, capacity integer, description character varying, is_available boolean, is_working boolean, amenities jsonb, images jsonb, total_records integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
